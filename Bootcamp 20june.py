...
2.Retrieve the names of all customers who have placed orders for dishes located in the same city as the customer named "Bob".                    SELECT cname
FROM customers
WHERE cid IN (
    SELECT DISTINCT cid
    FROM orders
...
3.Retrieve the names of all customers who have placed orders for dishes that have a price higher than the average price of dishes bought by each customer.                                                                                                                                                                                             SELECT cname
FROM customers c
WHERE EXISTS (
    SELECT 1
    FROM orders o
...
4.Retrieve the names of customers who have placed orders for dishes with a price higher than the average price of all dishes in the same city as the customer, and also display the total amount spent by each customer on such orders.                                                                                   SELECT c.cname, SUM(o.amt) AS total_amount_spent
FROM customers c
...
GROUP BY c.cname;
5.Retrieve the names of all customers along with the total amount they have spent on orders, including customers who have not placed any orders yet.                                                                                                                                                                                                                SELECT c.cname, COALESCE(SUM(o.amt), 0) AS total_amount_spent
FROM customers c
LEFT JOIN orders o ON c.cid = o.cid
GROUP BY c.cname;
6.Retrieve the names of dishes along with their prices and the ranking of each dish based on their prices, where the dishes are ranked in descending order of price.                                                                                                                                                                                            SELECT dname, price, RANK() OVER (ORDER BY price DESC) AS price_rank
FROM dishes;
